...
export class MusicRepository {
    ...
    static async save(data: Omit<Music, "id">): Promise<Music | null> {
        ...
        await client.execute(
            'INSERT INTO music (id, title, genre, artiste, souce, image_url) VALUES ( ?, ?, ?, ?, ?, ?);', 
            [v4(), data.title, data.genre, data.artiste, data.souce, data.image_url]
        )
        return { id, ...data }
    }
    ...
    static async find(id: string): Promise<Music | null> {
        ...
        return { id: data.id, title: data.title, genre: data.genre, artiste: data.artiste, souce: data.souce, image_url: data.image_url }
    }

    static async getAll(): Promise<Music[]> {
        const resul = await client.execute('SELECT * FROM music;')
    ...
    static async update(id: string, data: Omit<Music, "id">): Promise<Music | null> {
        await client.execute(
            'UPDATE music SET title = ?, genre = ?, artiste = ?, souce = ?, image_url = ? WHERE id = ?;', 
            [data.title, data.genre, data.artiste, data.souce, data.image_url, id]
        )
        return { id, ...data }
    }
