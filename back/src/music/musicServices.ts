...
export const getRecommendationsByCity = async (cityName: string): Promise<CustomResponseServiceType<Music[]>> => {
    let data: Music[] | null = null;
    let error: Error | null = null;
    try {
        const result = await MusicRepository.getRecommendationsByCity(cityName);
        if (result) {
            data = result;
        } else {
